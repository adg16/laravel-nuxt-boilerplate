...
- **Features**: `registration` + `resetPasswords` + `twoFactorAuthentication` (login/logout are always on). Profile/password self-service and email-verification are intentionally **off** (accounts are managed via the permissioned `UserController`). See **Two-factor authentication** below for how 2FA is wired.
- **Headless JSON responses** live in `app/Http/Responses/` and are bound in `FortifyServiceProvider::register`: `LoginResponse`/`RegisterResponse` return the `UserResource` (not Fortify's `{two_factor}` default); `LogoutResponse` returns a 200 `{message}`. **`PasswordResetLinkResponse` is bound to *both* the success and failure forgot-password contracts** so the two outcomes are byte-identical — this is what preserves **anti-enumeration** (unknown email still 200s with the same generic `passwords.reset_link_generic` message).
- **Actions** (`app/Actions/Fortify/`): `CreateNewUser` enforces the runtime `Setting::RegistrationEnabled` toggle (`abort(403)`) before validating — self-registration stays DB-toggleable, not a boot-time flag. `ResetUserPassword` also rotates `remember_token`. `authenticateUsing` (in the provider) throws `AuthenticationException` on bad credentials to **preserve the 401** contract (Fortify's default would be a 422).
...
- **Self-service `/security` page** (the app's only self-service surface) + a login challenge step in `pages/login.vue`; admins reset a locked-out user's 2FA (either method) via `UserController::resetTwoFactor` (`DELETE /api/users/{user}/two-factor`, `users.manage`).
