...
- **Features** are minimal: `registration` + `resetPasswords` (login/logout are always on). Profile/password self-service and email-verification are intentionally **off** (accounts are managed via the permissioned `UserController`). **Two-factor is off but ready** — re-enable `Features::twoFactorAuthentication(...)` and re-publish its migration (it was deleted since we don't use it).
