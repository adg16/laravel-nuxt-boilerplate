...
return [
     ...
     *
     * Pinned to `web` on purpose: the whole authenticated API runs under
     * `$middleware->statefulApi()`, which flips the *default* guard to `sanctum`
     * mid-request. We want the causer resolved from the SPA's session (`web`)
     * guard — the same guard App\Models\Concerns\Blameable stamps from — so
     * activity causers and created_by/updated_by never disagree.
     */
